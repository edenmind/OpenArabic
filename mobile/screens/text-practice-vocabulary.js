...
export const PracticeVocabulary = ({handleContinue}) => {
    ...
    const generateAnswerButton = (text, onPress, isCorrect = false) => {
        // remove everything in the beginning within [] and everything at the end within [] from text property
        const bracketsRemoved = text.replace(/\[.*?]/g, '').replace(/\(.*?\)/g, '').trim()
        return (
            <ButtonAnswer text={bracketsRemoved} onPress={onPress} correct={isCorrect} incorrect={!isCorrect}/>
        )
    }
